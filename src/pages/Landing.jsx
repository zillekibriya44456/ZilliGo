...
export default function Landing() {
  ...
  const [dynamicData, setDynamicData] = useState({ 
    tours: [], guides: [], liveStreams: [], activities: [], testimonials: [],
    stats: { tours: 0, guides: 0, cities: 0, travelers: 0 } 
  });
  ...
  const featuredTours = dynamicData.tours.slice(0, 4);
  const liveTours = dynamicData.liveStreams.map(l => ({ ...l, type: 'live' })).slice(0, 4);
          ...
          <div className="stats-bar-glow-card">
            {[
              ...
              <div key={idx} style={{ textAlign: 'center', flex: 1, minWidth: '130px' }}>
                <div style={{ fontSize: '1.4rem', fontWeight: 900, color: '#00F5D4', filter: 'drop-shadow(0 0 5px rgba(0, 245, 212, 0.4))' }}>
                  {stat.value}
                </div>
                <div style={{ fontSize: '0.65rem', color: 'rgba(248, 250, 252, 0.65)', fontWeight: 700, letterSpacing: '0.05em', marginTop: '4px' }}>
                  {stat.label}
                </div>
              </div>
            ))}
      ...
      <section className="section">
        <div className="container">
          ...
          <div className="grid-3">
            {dynamicData.testimonials && dynamicData.testimonials.map(t => (
              <div key={t.id} className="testimonial-card glass-card">
                ...
                <div className="testimonial-card__footer">
                  <img src={t.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(t.name)}`} alt={t.name} className="avatar" style={{ width: 40, height: 40 }} />
                  <div>
